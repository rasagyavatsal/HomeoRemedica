"""Replay v9/v10 candidate caches and check them against the recorded results."""
...
from compare_v9_retrieval import replay
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=ROOT / ".cache/evaluation/v10-comparison.json"
    )
    ...
    before, before_digest = load_evaluation_dataset(ROOT / "evaluation/v9/queries.json")
    after, after_digest = load_evaluation_dataset(ROOT / "evaluation/v10/queries.json")
    ...
    rankings = {"v9": replay(before, chunks, config), "v10": replay(after, chunks, config)}
    targets = tuple(
    ...
    for version, digest in (("v9", before_digest), ("v10", after_digest)):
    ...
    report = {
        ...
        "v9DatasetSha256": before_digest,
        "v10DatasetSha256": after_digest,
        "semanticQueryInstruction": after.semantic_query_instruction,
        "semanticScoreWeight": after.semantic_score_weight,
        "k": after.k,
        ...
        "queries": [
            {
                ...
                "v10Top8": list(rankings["v10"][i][: after.k]),
